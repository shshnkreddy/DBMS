...
void process_line( char *test_case )
{
	...
	if( !strcmp(command,"CREATEDB") ){
		...
		status = link_data(param1,atoi(param2),atoi(param3));
		if(status == ACADEMIA_SUCCESS)
		{
			TREPORT("PASS","");
		}
		else
		...
		expected_status = atoi(param3);
		struct PDS_LinkedKeySet linked_data;

		status = get_linked_data(param1,atoi(param2),&linked_data);
		if(status == expected_status)
		{
			TREPORT("PASS","");
		}
		...
		{
			sprintf(info,"Number of links are not equal, get_linked_data returned status %d",status);
			TREPORT("FAIL",info);
		}
	}
}
